...
2. Authentication에서 사용자를 만든 뒤 해당 사용자의 `profiles.is_approved`를 `true`로 설정합니다. 승인되지 않은 인증 사용자는 운영 데이터를 조회할 수 없습니다.
3. 이어서 `supabase/seed.sql`을 한 번 실행합니다. `ON CONFLICT`를 사용해 중복 생성을 방지합니다.
4. Authentication에서 이메일 로그인을 활성화하고 첫 사용자를 만듭니다.
5. 해당 사용자 id로 `profiles` 행을 승인하고 `role`을 `admin`으로 지정합니다.
6. `.env.local`에 공개 URL과 anon key만 입력합니다.

```sql
update public.profiles
set display_name = '관리자', role = 'admin', is_approved = true
...
핵심 분석·검색·필터·타임라인·CSV UI, Supabase 실데이터 조회와 승인 사용자 전용 RLS가 완료되어 있습니다. 실제 CRUD 폼, CSV 가져오기 미리보기, 세부 위험/마일스톤 편집, 사용자 관리 UI는 후속 구현 영역입니다.

향후에는 감사 트리거, 저장 필터, 다국어, 세부 보고서와 PNG 내보내기를 순차 적용합니다.
